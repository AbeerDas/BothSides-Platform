...
export const ArgumentCard = ({
  ...
  return (
    <div className={cn(
      ...
      depth > 0 && "ml-6 md:ml-8 border-l-2 pl-4 md:pl-6",
      depth > 0 && side === "for" ? "border-for-border" : depth > 0 && "border-against-border"
    )}>
      <Card className={cn(
        "p-5 md:p-6 transition-all duration-300 border shadow-card animate-fade-in relative", 
        ...
        <Button 
          ...
          className="absolute top-3 right-3 z-10 h-8 w-8 hover:bg-accent" 
        ...
        <div className="space-y-4 pr-10">
          <div className="space-y-3">
            {title && (
              <h3 className="font-serif font-semibold text-lg text-foreground tracking-wide">
                {title}
              </h3>
            )}
            {!isMinimized && (
              <>
                {subheading && (
                  <p className="text-sm font-body text-muted-foreground italic border-l-2 border-border pl-3">
                    {subheading}
                  </p>
                )}
          ...
          {!isMinimized && (
            <div className="pt-4 border-t border-border">
              <Button 
                ...
                disabled={isRefuting} 
                className={cn(
                  "gap-2 whitespace-nowrap font-sans text-xs uppercase tracking-wider transition-all duration-200",
                  side === "for" ? "hover:bg-against-bg hover:border-against-border" : "hover:bg-for-bg hover:border-for-border"
                )}
              >
      ...
      {isRefuting && (
        <div className="ml-6 md:ml-8 text-sm font-body text-muted-foreground italic animate-fade-in flex items-center gap-2">
