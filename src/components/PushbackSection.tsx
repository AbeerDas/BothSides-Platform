...
export const PushbackSection = ({
  statement
}: PushbackSectionProps) => {
  ...
  const handlePushback = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke("generate-pushback", {
        body: {
          statement,
  ...
  const loadingPhrases = ["Analyzing claim...", "Evaluating evidence...", "Structuring critique...", "Identifying weaknesses..."];
  ...
  return <Card className="p-6 bg-card border border-border">
      <div className="space-y-4">
        ...
        <div className="flex gap-3">
          <Input value={pushbackInput} onChange={e => setPushbackInput(e.target.value)} placeholder="Enter a claim to challenge..." className="flex-1 font-sans border-2" onKeyDown={e => {
          if (e.key === "Enter" && !isLoading) {
            handlePushback();
          }
        }} />
          <Button onClick={handlePushback} disabled={isLoading} className="gap-2 font-sans text-xs uppercase tracking-wider text-slate-50 bg-sky-800 hover:bg-sky-700">
            <MessageSquare className="h-4 w-4" />
            {isLoading ? "Analyzing..." : "Pushback"}
          </Button>
        </div>

        {isLoading && <div className="animate-in fade-in duration-300">
            <p className="text-sm font-body text-muted-foreground italic">
              {loadingPhrases[currentPhrase]}
            </p>
          </div>}

        {pushbackResult && !isLoading && <Card className="p-6 bg-muted border border-border animate-in fade-in slide-in-from-top-2 duration-300">
